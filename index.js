...
function drinkWater(currentThirstLevel) {
  for(let i = 0; i < currentThirstLevel; currentThirstLevel -= 1) {
...
function printNameAndPhones(array) {
  array.forEach(function (arr) {
    ...
    console.log("Cell: " + arr["phones"]["cell"]);
    console.log("Cell: " + arr["phones"]["office"]);
  })
}
...
function takeATicketNumber(line) {
  let num = `${line.length + 1}`
  line.push(num)

  return `Welcome. You are ticket number ${line.length}`
  }

  //I know this is not entirely correct, bc I can't get the line to persist after the two nowServing calls.
